...
void ModuleBase::SendIndexCmd(uint8_t index) {}

void ModuleBase::SetRecordingState(uint8_t recordingState, uint16_t delay) {}

void ModuleBase::SetOSD(mspPacket_t *packet) {}

void ModuleBase::SendHeadTrackingEnableCmd(bool enable) {}

void ModuleBase::SetRTC() {}

void ModuleBase::SendLinkTelemetry(uint8_t *rawCrsfPacket) {}

void ModuleBase::SendBatteryTelemetry(uint8_t *rawCrsfPacket) {}

void ModuleBase::Loop(uint32_t now) {}

void MSPModuleBase::Loop(uint32_t now) {
  ...
  while (m_port->available()) {
    ...
    if (msp.processReceivedByte(data)) {
      ...
      if (packet->function == MSP_ELRS_BACKPACK_SET_MODE) {
        ...
        sendResponse(MSP_ELRS_BACKPACK_GET_VERSION, backpackVersion,
                     strlen((const char *)backpackVersion));
...
void MSPModuleBase::sendResponse(uint16_t function, const uint8_t *response,
